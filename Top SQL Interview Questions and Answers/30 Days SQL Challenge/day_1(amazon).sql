...
SELECT COUNTRY,PRODUCT_NAME,PRICE,ROW_NUMBER() OVER(PARTITION BY COUNTRY ORDER BY PRICE DESC) AS RN
FROM PRODUCTS p
INNER JOIN SUPPLIERS S
ON s.supplier_id=p.supplier_id)
SELECT COUNTRY,PRODUCT_NAME,PRICE
 FROM COUNTRY_WISE WHERE RN=1;
...
SELECT c.customer_name ,SUM(Amount) AS TOTAL_AMOUNT FROM Customers c

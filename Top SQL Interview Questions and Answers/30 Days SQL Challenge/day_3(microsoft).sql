...
WITH highest_salary_dep AS (
SELECT *,ROW_NUMBER() OVER(PARTITION BY department ORDER BY salary DESC) AS rn  FROM Salaries)
SELECT (salary-LEAD(salary) OVER()) FROM highest_salary_dep WHERE rn=1 LIMIT 1;